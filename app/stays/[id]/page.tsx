...
export default function StayDetailPage() {
  ...
  const listing = useAppStore((s) => s.stayListings.find((l) => l.id === id));
      ...
      <div className="grid gap-12 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2">
          ...
          <section aria-labelledby="rv-fit-heading">
            <h2 id="rv-fit-heading" className="mb-4 text-lg font-semibold">
              RV Fit
            </h2>
            <div className="grid grid-cols-3 gap-4">
              ...
              <FitItem
                ...
                label="Slide-outs"
                value={listing.slideOutsAllowed ? "Yes" : "No"}
              />
              <FitItem
                icon={<MoveRight className="h-5 w-5" />}
                label="Pull-through"
                value={listing.pullThrough ? "Yes" : "No"}
              />
            </div>
          </section>
          ...
          <section aria-labelledby="hookups-heading">
            <h2 id="hookups-heading" className="mb-4 text-lg font-semibold">
              Hookups &amp; Features
            </h2>
            <div className="grid gap-3 sm:grid-cols-2">
              <HookupRow
                ...
                label="Electric"
                value={electricLabel}
                available={listing.electric !== "NONE"}
              />
              <HookupRow
                icon={<Droplets className="h-5 w-5 text-blue-500" />}
                label="Water"
                value={listing.water ? "Yes" : "No"}
                available={listing.water}
              />
              <HookupRow
                icon={<Pipette className="h-5 w-5 text-gray-500" />}
                label="Sewage"
                value={listing.sewage ? "Yes" : "No"}
                available={listing.sewage}
              />
              <HookupRow
                icon={<Flame className="h-5 w-5 text-orange-500" />}
                label="Gas (propane)"
                value={listing.gas ? "Yes" : "No"}
                available={listing.gas}
              />
            </div>
          </section>
          ...
          <section aria-labelledby="rules-heading">
            ...
            <ul className="space-y-1 text-sm text-muted-foreground">
              <li className="flex items-center gap-2">
                ...
                Minimum stay: {listing.minStayNights} night
                {listing.minStayNights !== 1 && "s"}
              </li>
              <li className="flex items-center gap-2">
                <CalendarDays className="h-4 w-4" aria-hidden="true" />
                Maximum stay: {listing.maxStayNights} night
                {listing.maxStayNights !== 1 && "s"}
              </li>
            </ul>
          </section>
          ...
          >
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Joined{" "}
                {new Date(listing.host.joinedDate).toLocaleDateString(
                  "en-US",
...
function HookupRow({
  ...
  return (
    <div
      className={`flex items-center gap-3 rounded-lg p-2 ${available ? "" : "opacity-40"}`}
    >
      {icon}
    ...
    <Card className="shadow-lg">
      <CardContent className="space-y-5 p-6">
        ...
        <p className="text-xs text-muted-foreground">
          {listing.minStayNights}–{listing.maxStayNights} night stay
        </p>
      </CardContent>
    </Card>
  );
